"""
Script di test per il robot auto-bilanciante con Deep Reinforcement Learning.
"""
import argparse
import gymnasium as gym
from stable_baselines3 import SAC
from stable_baselines3.common.monitor import Monitor
from src.env.self_balancing_robot_env.self_balancing_robot_env import (
    SelfBalancingRobotEnv  # Assicura il registry dell'env
)
...
if __name__ == "__main__":
    ...
    print(f"  - Ambiente: {args.environment_path}")
    print(f"  - Tempo massimo: {args.max_time}")
    print(f"  - Step di test: {args.test_steps}")
    print()
    
    env = make_env(environment_path=args.environment_path, max_time=args.max_time)

    # Carica il modello addestrato
    model = SAC.load(args.path, env=env)
    print(f"Modello caricato da: {args.path}")
